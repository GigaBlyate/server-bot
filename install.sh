#!/bin/bash

# =============================================
# 🚀 УСТАНОВЩИК/УДАЛИТЕЛЬ БОТА
# =============================================

set -e

# Цвета
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
PURPLE='\033[0;35m'
CYAN='\033[0;36m'
WHITE='\033[1;37m'
NC='\033[0m'

# Конфигурация
REPO="GigaBlyate/server-bot"
INSTALL_DIR="$HOME/server-bot"
VENV_DIR="$INSTALL_DIR/venv"
LOG_FILE="$HOME/install-bot.log"

show_logo() {
    clear
    echo -e "${PURPLE}"
    echo "╔════════════════════════════════════════════════════╗"
    echo "║                                                    ║"
    echo "║    ██████  ██████  ████████     ██████  ██████     ║"
    echo "║   ██░░░░██░░██░░██░░██░░██    ██░░░░██░░██░░██    ║"
    echo "║  ██    ░░  ██  ░░  ██  ░█   ██    ░░  ██  ░█    ║"
    echo "║  ██        ██      █████    ██        █████      ║"
    echo "║  ██    ██  ██      ██  ██   ██    ██  ██  ██     ║"
    echo "║  ░░██████  ██████  ████████  ░░██████  ████████   ║"
    echo "║    ░░░░░░  ░░░░░░  ░░░░░░░░    ░░░░░░  ░░░░░░░░   ║"
    echo "║                                                    ║"
    echo "║              🤖 SERVER BOT v2.1.0 🤖              ║"
    echo "║                                                    ║"
    echo "║                by ${CYAN}GigaBlyate${PURPLE}                    ║"
    echo "║     https://github.com/GigaBlyate/server-bot      ║"
    echo "╚════════════════════════════════════════════════════╝"
    echo -e "${NC}"
}

# Функция полного удаления бота
uninstall_bot() {
    echo -e "\n${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${RED}  🗑️  ПОЛНОЕ УДАЛЕНИЕ БОТА${NC}"
    echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    
    echo -e "${YELLOW}⚠️  ВНИМАНИЕ! Это удалит:${NC}"
    echo "   • Все файлы бота из $INSTALL_DIR"
    echo "   • Systemd сервис server-bot"
    echo "   • Базу данных и логи"
    echo ""
    read -p "Вы точно хотите удалить бота? (y/n): " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        echo -e "${GREEN}Удаление отменено${NC}"
        return 1
    fi
    
    echo -e "\n${YELLOW}⏳ Останавливаю сервис...${NC}"
    sudo systemctl stop server-bot 2>/dev/null || true
    sudo systemctl disable server-bot 2>/dev/null || true
    
    echo -e "${YELLOW}⏳ Удаляю systemd сервис...${NC}"
    sudo rm -f /etc/systemd/system/server-bot.service
    sudo systemctl daemon-reload
    
    echo -e "${YELLOW}⏳ Удаляю файлы бота...${NC}"
    rm -rf "$INSTALL_DIR"
    
    echo -e "${YELLOW}⏳ Удаляю логи установки...${NC}"
    rm -f "$LOG_FILE"
    rm -f "$HOME/bot" 2>/dev/null
    rm -f "$HOME/bot-menu.sh" 2>/dev/null
    rm -f "$HOME/bot-commands.txt" 2>/dev/null
    
    echo -e "${GREEN}✅ Бот полностью удален с сервера!${NC}"
    exit 0
}

# Функция установки Python библиотек
install_python_deps() {
    echo -e "\n${BLUE}📚 Установка Python библиотек...${NC}"
    source "$VENV_DIR/bin/activate"
    
    # Основные библиотеки
    local packages=(
        "python-telegram-bot==20.7"
        "psutil==5.9.5"
        "aiohttp==3.9.1"
        "cryptography==41.0.7"
    )
    
    local total=${#packages[@]}
    local current=0
    
    for pkg in "${packages[@]}"; do
        current=$((current + 1))
        echo -ne "\r${CYAN}⏳ Установка [$current/$total]: $pkg${NC}"
        pip install -q "$pkg" >> "$LOG_FILE" 2>&1
        sleep 0.3
    done
    echo -e "\r${GREEN}✅ Все библиотеки установлены!${NC}                       "
}

# =============================================
# ГЛАВНОЕ МЕНЮ
# =============================================

show_logo

echo -e "\n${WHITE}Добро пожаловать!${NC}"
echo ""
echo "╔════════════════════════════════════════╗"
echo "║  Выберите действие:                    ║"
echo "╠════════════════════════════════════════╣"
echo "║  ${GREEN}1) Установить бота${NC}                     ║"
echo "║  ${RED}2) Удалить бота${NC}                       ║"
echo "║  ${YELLOW}3) Выйти${NC}                               ║"
echo "╚════════════════════════════════════════╝"
echo ""
read -p "Ваш выбор (1-3): " main_choice

case $main_choice in
    2)
        uninstall_bot
        exit 0
        ;;
    3)
        echo -e "${YELLOW}Выход...${NC}"
        exit 0
        ;;
    1)
        echo -e "${GREEN}Продолжаем установку...${NC}"
        ;;
    *)
        echo -e "${RED}Неверный выбор. Выход.${NC}"
        exit 1
        ;;
esac

# =============================================
# УСТАНОВКА
# =============================================

# 1. СИСТЕМНЫЕ ЗАВИСИМОСТИ
echo -e "\n${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${CYAN}  🔧 ПРОВЕРКА СИСТЕМЫ${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"

# Git
if ! command -v git &> /dev/null; then
    echo -e "${YELLOW}⚠️ Устанавливаю Git...${NC}"
    sudo apt update >> "$LOG_FILE" 2>&1
    sudo apt install -y git >> "$LOG_FILE" 2>&1
    echo -e "${GREEN}✅ Git установлен${NC}"
else
    echo -e "${GREEN}✅ Git уже установлен${NC}"
fi

# Python
if ! command -v python3 &> /dev/null; then
    echo -e "${YELLOW}⚠️ Устанавливаю Python3...${NC}"
    sudo apt install -y python3 python3-pip python3-venv >> "$LOG_FILE" 2>&1
    echo -e "${GREEN}✅ Python3 установлен${NC}"
else
    echo -e "${GREEN}✅ Python3 уже установлен${NC}"
fi

# SQLite
if ! command -v sqlite3 &> /dev/null; then
    echo -e "${YELLOW}⚠️ Устанавливаю SQLite3...${NC}"
    sudo apt install -y sqlite3 >> "$LOG_FILE" 2>&1
    echo -e "${GREEN}✅ SQLite3 установлен${NC}"
else
    echo -e "${GREEN}✅ SQLite3 уже установлен${NC}"
fi

# 2. КЛОНИРОВАНИЕ
echo -e "\n${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${CYAN}  📥 ЗАГРУЗКА БОТА${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"

if [ -d "$INSTALL_DIR" ]; then
    echo -e "${YELLOW}⚠️ Папка $INSTALL_DIR уже существует${NC}"
    read -p "Удалить и переустановить? (y/n): " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        rm -rf "$INSTALL_DIR"
    else
        echo -e "${YELLOW}Пропускаю клонирование${NC}"
        SKIP_CLONE=1
    fi
fi

if [ ! -d "$INSTALL_DIR" ] && [ "$SKIP_CLONE" != 1 ]; then
    echo -e "Клонирование репозитория..."
    git clone "https://github.com/$REPO.git" "$INSTALL_DIR" >> "$LOG_FILE" 2>&1
    echo -e "${GREEN}✅ Репозиторий загружен${NC}"
fi

cd "$INSTALL_DIR"

# 3. ВИРТУАЛЬНОЕ ОКРУЖЕНИЕ
echo -e "\n${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${CYAN}  🐍 НАСТРОЙКА PYTHON${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"

if [ ! -d "$VENV_DIR" ]; then
    echo -e "Создание виртуального окружения..."
    python3 -m venv "$VENV_DIR" >> "$LOG_FILE" 2>&1
    echo -e "${GREEN}✅ Виртуальное окружение создано${NC}"
else
    echo -e "${GREEN}✅ Виртуальное окружение уже есть${NC}"
fi

install_python_deps

# 4. БАЗА ДАННЫХ
echo -e "\n${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${CYAN}  🗄️  НАСТРОЙКА БАЗЫ ДАННЫХ${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"

if [ ! -f "vps_data.db" ]; then
    sqlite3 vps_data.db << EOF
CREATE TABLE IF NOT EXISTS settings (
    key TEXT PRIMARY KEY,
    value TEXT NOT NULL
);
INSERT OR IGNORE INTO settings (key, value) VALUES 
    ('cpu_threshold', '30'),
    ('ram_threshold', '85'),
    ('disk_threshold', '85'),
    ('monitor_interval', '30'),
    ('enable_daily_report', 'false'),
    ('enable_fail2ban_alerts', 'false'),
    ('compact_mode', 'false');
EOF
    echo -e "${GREEN}✅ База данных создана${NC}"
else
    echo -e "${GREEN}✅ База данных уже существует${NC}"
fi

# 5. SYSTEMD СЕРВИС
echo -e "\n${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${CYAN}  ⚙️  НАСТРОЙКА АВТОЗАПУСКА${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"

sudo tee /etc/systemd/system/server-bot.service > /dev/null << EOF
[Unit]
Description=Server Bot
After=network.target

[Service]
Type=simple
User=$USER
WorkingDirectory=$INSTALL_DIR
ExecStart=$VENV_DIR/bin/python $INSTALL_DIR/bot.py
Restart=always

[Install]
WantedBy=multi-user.target
EOF

sudo systemctl daemon-reload
sudo systemctl enable server-bot >> "$LOG_FILE" 2>&1
echo -e "${GREEN}✅ Сервис создан и добавлен в автозагрузку${NC}"

# 6. ЗАПУСК
echo -e "\n${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${CYAN}  🚀 ЗАПУСК БОТА${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"

sudo systemctl start server-bot
sleep 3

if sudo systemctl is-active --quiet server-bot; then
    echo -e "${GREEN}✅ Бот успешно запущен!${NC}"
else
    echo -e "${RED}❌ Ошибка запуска. Проверьте логи:${NC}"
    echo "sudo journalctl -u server-bot -n 20 --no-pager"
fi

# 7. МЕНЮ УПРАВЛЕНИЯ
cat > "$HOME/bot" << 'EOF'
#!/bin/bash
PURPLE='\033[0;35m'
CYAN='\033[0;36m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
RED='\033[0;31m'
NC='\033[0m'

echo -e "${PURPLE}"
echo "╔════════════════════════════════════════╗"
echo "║       🤖 УПРАВЛЕНИЕ БОТОМ 🤖         ║"
echo "╠════════════════════════════════════════╣"
echo "║ 1. Статус                             ║"
echo "║ 2. Логи                                ║"
echo "║ 3. Перезапустить                       ║"
echo "║ 4. Остановить                          ║"
echo "║ 5. Запустить                           ║"
echo "║ 6. Удалить бота                         ║"
echo "║ 7. Выйти                               ║"
echo "╚════════════════════════════════════════╝${NC}"
read -p "Выберите действие: " cmd
case $cmd in
    1) sudo systemctl status server-bot ;;
    2) sudo journalctl -u server-bot -f ;;
    3) sudo systemctl restart server-bot ;;
    4) sudo systemctl stop server-bot ;;
    5) sudo systemctl start server-bot ;;
    6) cd ~ && wget -O uninstall.sh https://raw.githubusercontent.com/GigaBlyate/server-bot/main/install.sh && bash uninstall.sh ;;
    7) exit 0 ;;
esac
EOF

chmod +x "$HOME/bot"

# 8. ФИНАЛ
echo -e "\n${GREEN}╔════════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║  🎉 УСТАНОВКА ЗАВЕРШЕНА!                           ║${NC}"
echo -e "${GREEN}║                                                    ║${NC}"
echo -e "${GREEN}║  Команды для управления:                           ║${NC}"
echo -e "${GREEN}║  ${CYAN}./bot${GREEN} - полное меню                          ║${NC}"
echo -e "${GREEN}║  ${CYAN}sudo systemctl status server-bot${GREEN} - статус  ║${NC}"
echo -e "${GREEN}║  ${CYAN}sudo journalctl -u server-bot -f${GREEN} - логи   ║${NC}"
echo -e "${GREEN}╚════════════════════════════════════════════════════╝${NC}"
