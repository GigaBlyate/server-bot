...
def _write_reboot_marker(admin_id: str, first_name: str, action: str = 'reboot') -> None:
    payload = {'admin_id': admin_id, 'first_name': first_name, 'action': action}
...
async def _perform_bot_update(query, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    _write_reboot_marker(str(query.message.chat_id), first_name, action='bot_restart_after_update')

    await query.message.edit_text(
        '✅ <b>Обновление завершено</b>\n\n'
        'Код бота обновлён. Бот перезапускается…\n'
        'После запуска я автоматически открою главное меню.\n\n'
        f'<code>{escape_html(details[-2000:])}</code>',
        parse_mode='HTML',
    )

    await asyncio.sleep(0.8)

    code, out, err = await safe_run_command(_root_helper('restart', 'server-bot.service'), timeout=10)
    if code != 0:
        await safe_run_command(_root_helper('restart-bot'), timeout=10)
    return
